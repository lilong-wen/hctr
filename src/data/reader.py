...
class Dataset():
    ...
    def _washington(self):
        ...
        return dataset
    def _casia(self):
        """casia dataset reader"""

        pt_path = os.path.join(self.source, "cv1")

        paths = {"train": open(os.path.join(pt_path, "train.txt")).read().splitlines(),
                 "valid": open(os.path.join(pt_path, "valid.txt")).read().splitlines(),
                 "test": open(os.path.join(pt_path, "test.txt")).read().splitlines()}

        lines = open(os.path.join(self.source, "label.txt")).read().splitlines()
        gt_dict = dict()

        for line in lines:
            splitted = line.split()
            # TODO
            # convert chinese character to numerical representation

            gt_dict[splitted[0]] = splitted[1]

        img_path = os.path.join(self.source, "raw_data_images")
        dataset = dict()
