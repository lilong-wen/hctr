...
def read_from_agrl(dgrls):
    if not os.path.exists(dgrls):
        print('DGRL not exis!')
        return

    root_dir = os.path.dirname(dgrls)
    # f1 = open('./test_label.txt', 'w')
    f1 = open(os.path.join(root_dir, "label.txt"), 'w')

    for root, dirs, files in os.walk(dgrls):
        for dgrl in files:
            dir_name = root
            base_name = dgrl
            ...
            with open(os.path.join(root,dgrl), 'rb') as f:
                # 读取表头尺寸
                ...
                for k in range(line_num):
                    ...
                    # 图片路径
                    pic_path = os.path.join(image_dir, base_name.replace('.dgrl', '_' + str(k) + '.jpg'))
                    img_path = base_name.replace('.dgrl', '_' + str(k)+" ")
                    # 写入label
                    # f1.write(pic_path+'\t'+label+'\n')
                    f1.write(img_path+'\t'+label+'\n')
                    bitmap_file = os.path.join(image_dir, base_name.replace('.dgrl', '_' + str(k) + '.jpg'))
                    cv.imwrite(bitmap_file, bitmap)

if __name__ == "__main__":
    read_from_agrl('HWDB2.0Test')
